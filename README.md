...
## Example of the correct encoding using the Erlang ASN.1 compiler
The Erlang OTP ASN.1 compiler is capable of generating and consuming the correct UPER encoding for bitstrings with extensions.  
...
1. The UPER encoding produced by the V1 andV2 encoders must be identical if no bits outside the 
...
The codec produced by the Erland ASN.1 compiler meets all three requirements.
...
Erlang codecs for V1 and V2 were generated by running the following in the Erlang shell:
```erlang
...
Encode the basic fruit salad to UPER with both the V1 and V2 codecs and save the UPER to files.
...
Convert the UPER to hex and save it to files
```erlang
...
The V1 codec can decode the V1 UPER and vice versa.  Both of thesse succeed:
```erlang
...
#### Test 2.
Let's see what the UPER looks like for when extensions are set.
...
The UPER with extention is the following:
```
hex: 82FDFC
